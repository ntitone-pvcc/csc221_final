...
def compound_check (compound_pick):
    while True:
        compound_pick = input("please selected at what frequency you would like to compound to be at. /n monthly(1), quarterly(2), or yearly(3). ")
        if (compound_pick == "1" ):
            x = int(compound_pick)
            if (x == 1):
...
def invest_length(x):
    ...
    while True:
        y = int(input(f"please select how many {z} you would like for "))
        if (x == 1):
...
#Length = int(input("how long would you like to invest for? please enter the total number of days you would ")) #User picks how long to invest for
#print(f"you have chosen to invest your money for {Length} days")
